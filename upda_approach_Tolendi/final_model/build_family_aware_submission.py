...
def compute_question_meta(df: pd.DataFrame) -> pd.DataFrame:
    ...
    combined = (out['context'].fillna('') if 'context' in out.columns else '') + ' ' + (out['question_text'].fillna('') if 'question_text' in out.columns else '')
    ...
    out['headline_title'] = combined.map(parse_headline_title)
    return out
