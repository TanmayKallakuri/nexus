"""
Leakage-aware Tolendi V2 bootstrap trainer.

Run from repo root:
  python scripts/bootstrap_ensemble_tolendi_v2.py
"""
...
QUESTION_EMBED_V2_PATH = OUTPUT_DIR / "question_embeddings_v2.csv"
PCA_MODEL_PATH = OUTPUT_DIR / "pca_model.pkl"
MODEL_BUNDLE_V2_PATH = OUTPUT_DIR / "bootstrap_ensemble_v2.pkl"
VALIDATION_SUMMARY_V2_PATH = OUTPUT_DIR / "validation_summary_v2.json"
VALIDATION_BY_BLOCK_V2_PATH = OUTPUT_DIR / "validation_by_block_v2.csv"
VALIDATION_BY_QUESTION_V2_PATH = OUTPUT_DIR / "validation_by_question_v2.csv"
HOLDOUT_PREDICTIONS_V2_PATH = OUTPUT_DIR / "holdout_predictions_v2.csv"
TRAINING_REPORT_V2_PATH = OUTPUT_DIR / "training_report_v2.txt"
...
def load_source_frames() -> Dict[str, object]:
    print("=" * 72)
    print("TOLENDI V2 - LEAKAGE-AWARE BOOTSTRAP BUILD")
    print("=" * 72)
...
def build_question_feature_table(
    ...
    q_feat.to_csv(QUESTION_EMBED_V2_PATH, index=False)
    with PCA_MODEL_PATH.open("wb") as f:
        pickle.dump(pca, f)
    ...
    print(f"  saved question features: {QUESTION_EMBED_V2_PATH}")
...
def run_validation(
    ...
    print("\n[3/8] Running leakage-aware held-out-question validation")
    ...
    person_feat_train = build_person_feature_frame(
        master_train,
        all_person_ids,
    ...
    question_metrics.to_csv(VALIDATION_BY_QUESTION_V2_PATH, index=False)
    block_metrics.to_csv(VALIDATION_BY_BLOCK_V2_PATH, index=False)
    scored_df.to_csv(HOLDOUT_PREDICTIONS_V2_PATH, index=False)
    VALIDATION_SUMMARY_V2_PATH.write_text(
...
def train_full_models(
    ...
    print("\n[4/8] Training full routed ensemble")

    person_feat_full = build_person_feature_frame(
        master,
        all_person_ids,
        reference_profile_cols,
    )
    ...
    with MODEL_BUNDLE_V2_PATH.open("wb") as f:
        pickle.dump(bundle, f)

    print(f"  saved model bundle: {MODEL_BUNDLE_V2_PATH}")
    return bundle
...
def write_training_report(validation: Dict[str, object]) -> None:
    ...
    lines.extend(
        [
            "",
            "Red flag checks:",
            f"  Mean Pearson r < 0.25: {summary['mean_pearson_r'] < 0.25}",
            f"  Variance ratio < 0.30: {summary['variance_ratio'] < 0.30}",
            f"  Accuracy < 0.67: {summary['accuracy'] < 0.67}",
        ]
    )

    TRAINING_REPORT_V2_PATH.write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    try:
        ...
        print("\n[6/8] Key outputs")
        print(f"  summary: {VALIDATION_SUMMARY_V2_PATH}")
        print(f"  by block: {VALIDATION_BY_BLOCK_V2_PATH}")
        print(f"  by question: {VALIDATION_BY_QUESTION_V2_PATH}")
        print(f"  holdout predictions: {HOLDOUT_PREDICTIONS_V2_PATH}")
        print(f"  bundle: {MODEL_BUNDLE_V2_PATH}")
        print(f"  report: {TRAINING_REPORT_V2_PATH}")

        print("\n[7/8] Validation snapshot")
        print(f"  accuracy: {validation['summary']['accuracy']:.4f}")
        print(f"  mean Pearson r: {validation['summary']['mean_pearson_r']:.4f}")
