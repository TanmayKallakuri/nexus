"""
Leakage-aware Tolendi V4.1 bootstrap trainer.

V4.1 uses:
  - repaired Model 1 person features
  - fold-specific K=10 person embeddings
  - two top-level routes (Personality vs Non-Personality)
  - binary / non-binary handling inside Non-Personality
  - analytical category dummies for Non-Personality
  - post-prediction calibration sweep

Run from repo root:
  python scripts/bootstrap_ensemble_tolendi_v4_1.py
"""
...
PERSON_PROFILE_REFERENCE_PATH = PROJECT_ROOT / "outputs" / "person_response_profiles_repaired.csv"
...
QUESTION_EMBED_V4_1_PATH = OUTPUT_DIR / "question_embeddings_v4_1.csv"
QUESTION_PCA_MODEL_V4_1_PATH = OUTPUT_DIR / "pca_model_v4_1.pkl"
MODEL_BUNDLE_V4_1_PATH = OUTPUT_DIR / "bootstrap_ensemble_v4_1.pkl"
VALIDATION_SUMMARY_V4_1_PATH = OUTPUT_DIR / "validation_summary_v4_1.json"
VALIDATION_BY_BLOCK_V4_1_PATH = OUTPUT_DIR / "validation_by_block_v4_1.csv"
VALIDATION_BY_QUESTION_V4_1_PATH = OUTPUT_DIR / "validation_by_question_v4_1.csv"
HOLDOUT_PREDICTIONS_V4_1_PATH = OUTPUT_DIR / "holdout_predictions_v4_1.csv"
TRAINING_REPORT_V4_1_PATH = OUTPUT_DIR / "training_report_v4_1.txt"
CALIBRATION_SWEEP_V4_1_PATH = OUTPUT_DIR / "calibration_sweep_v4_1.csv"
CALIBRATION_PARAMS_V4_PATH = OUTPUT_DIR / "calibration_params_v4.json"
...
HOLDOUT_FRAC = 0.2
SEED = 42
PERSON_EMBEDDING_K = 10
CALIBRATION_STRETCH_FACTORS = [1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]

PERSONALITY_BOOTSTRAPS = 12
NON_PERSONALITY_MULTI_BOOTSTRAPS = 10
...
def load_source_frames() -> Dict[str, object]:
    print("=" * 72)
    print("TOLENDI V4.1 - LEAKAGE-AWARE BOOTSTRAP BUILD")
    print("=" * 72)
...
def build_question_feature_table(
    ...
    q_feat.to_csv(QUESTION_EMBED_V4_1_PATH, index=False)
    with QUESTION_PCA_MODEL_V4_1_PATH.open("wb") as f:
        pickle.dump(pca, f)
    ...
    print(f"  saved question features: {QUESTION_EMBED_V4_1_PATH}")
    print(f"  saved PCA model: {QUESTION_PCA_MODEL_V4_1_PATH}")
...
def calibrate_predictions(predictions_df: pd.DataFrame, stretch_factor: float) -> pd.DataFrame:
    calibrated = predictions_df.copy()
    for question_id, group in calibrated.groupby("question_id", sort=False):
        idx = group.index
        preds = group["predicted"].to_numpy(dtype=np.float32)
        q_mean = float(preds.mean())
        q_std = float(preds.std())
        if q_std < 0.01:
            continue
        stretched = q_mean + stretch_factor * (preds - q_mean)
        num_opts = float(group["num_options"].iloc[0])
        calibrated.loc[idx, "predicted"] = np.clip(stretched, 1.0, num_opts)
    return calibrated


def sweep_calibration(predictions_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, float]]:
    sweep_rows: List[Dict[str, float]] = []
    best_df = predictions_df.copy()
    best_summary = score_predictions(best_df)[2]
    best_sf = 1.0
    best_valid_r = -np.inf

    for stretch_factor in CALIBRATION_STRETCH_FACTORS:
        calibrated = calibrate_predictions(predictions_df, stretch_factor)
        _, _, summary = score_predictions(calibrated)
        summary["stretch_factor"] = float(stretch_factor)
        sweep_rows.append(summary)

        if summary["accuracy"] >= 0.70 and summary["mean_pearson_r"] > best_valid_r:
            best_valid_r = summary["mean_pearson_r"]
            best_df = calibrated
            best_sf = float(stretch_factor)
            best_summary = summary

    if best_valid_r == -np.inf:
        best_sf = 1.0
        best_df = predictions_df.copy()
        best_summary = score_predictions(best_df)[2]
        best_summary["stretch_factor"] = best_sf

    sweep_df = pd.DataFrame(sweep_rows).sort_values("stretch_factor").reset_index(drop=True)
    return best_df, sweep_df, {
        "stretch_factor": best_sf,
        "best_accuracy": float(best_summary["accuracy"]),
        "best_r": float(best_summary["mean_pearson_r"]),
        "best_variance_ratio": float(best_summary["variance_ratio"]),
    }
...
def route_label(df: pd.DataFrame) -> pd.Series:
    labels = pd.Series("non_personality_multi", index=df.index, dtype="object")
    labels[df["block_name"] == "Personality"] = "personality"
    labels[(df["block_name"] != "Personality") & (df["num_options"] == 2)] = "non_personality_binary"
    return labels
...
ANALYTICAL_CATEGORY_MAP = {
    ...
    "demographics": {
        "QID11", "QID12", "QID13", "QID14", "QID15", "QID16",
        "QID17", "QID18", "QID19", "QID20", "QID21", "QID22",
        "QID23", "QID24",
    },
}

ANALYTICAL_CATEGORY_COLS = [
    "cat_verbal",
    "cat_finlit",
    "cat_logical",
    "cat_games",
...
def add_analytical_category_features(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out["category"] = out["parent_question_id"].apply(get_analytical_category)
    dummies = pd.get_dummies(out["category"], prefix="cat", dtype=np.float32)
    for col in ANALYTICAL_CATEGORY_COLS:
        if col not in dummies.columns:
            dummies[col] = 0.0
    dummies = dummies[ANALYTICAL_CATEGORY_COLS]
...
def run_validation(
    ...
    shared_feature_cols = build_feature_columns(person_feat_train, question_feature_cols)
    ...
    route_specs = [
        ...
        (
            "non_personality_binary",
            train_routed["route_name"] == "non_personality_binary",
            BINARY_BOOTSTRAPS,
            "binary",
        ),
        (
            "non_personality_multi",
            train_routed["route_name"] == "non_personality_multi",
    ...
    for route_name, train_mask, n_bootstraps, route_type in route_specs:
        ...
        if route_name != "personality":
            train_model_df = add_analytical_category_features(train_model_df)
            test_model_df = add_analytical_category_features(test_model_df)
            feature_cols = shared_feature_cols + ANALYTICAL_CATEGORY_COLS
        else:
    ...
    print(f"  pre-cal accuracy: {summary['accuracy']:.4f}")
    print(f"  pre-cal mean Pearson r: {summary['mean_pearson_r']:.4f}")
    print(f"  pre-cal variance ratio: {summary['variance_ratio']:.4f}")

    calibrated_df, calibration_sweep_df, calibration_params = sweep_calibration(scored_df)
    cal_question_metrics, cal_block_metrics, cal_summary = score_predictions(calibrated_df)
    cal_summary.update(
        {
            "holdout_fraction": HOLDOUT_FRAC,
            "holdout_question_count": int(len(holdout_qids)),
            ...
            "person_embedding_info": embedding_info,
            "route_reports": model_reports,
            "best_stretch_factor": calibration_params["stretch_factor"],
        }
    )

    print(f"  best stretch factor: {calibration_params['stretch_factor']}")
    print(f"  post-cal accuracy: {cal_summary['accuracy']:.4f}")
    print(f"  post-cal mean Pearson r: {cal_summary['mean_pearson_r']:.4f}")
    print(f"  post-cal variance ratio: {cal_summary['variance_ratio']:.4f}")

    return {
        "pre_summary": summary,
        "pre_question_metrics": question_metrics,
        "pre_block_metrics": block_metrics,
        "pre_scored_df": scored_df,
        "summary": cal_summary,
        "question_metrics": cal_question_metrics,
        "block_metrics": cal_block_metrics,
        "scored_df": calibrated_df,
        "calibration_sweep": calibration_sweep_df,
...
def train_full_models(
    ...
    print(f"\n[4/8] Training full routed ensemble (K={PERSON_EMBEDDING_K})")
    ...
    person_emb_output_path = OUTPUT_DIR / f"person_embeddings_v4_1_k{PERSON_EMBEDDING_K}.csv"
    person_emb_full.to_csv(person_emb_output_path, index=False)
    shared_feature_cols = build_feature_columns(person_feat_full, question_feature_cols)
    ...
    bundle: Dict[str, object] = {
        ...
        "analytical_category_cols": ANALYTICAL_CATEGORY_COLS,
    ...
    route_specs = [
        ...
        (
            "non_personality_binary",
            routed["route_name"] == "non_personality_binary",
            BINARY_BOOTSTRAPS,
            "binary",
        ),
        (
            "non_personality_multi",
            routed["route_name"] == "non_personality_multi",
            NON_PERSONALITY_MULTI_BOOTSTRAPS,
            "regression",
        ),
    ]
    ...
    with MODEL_BUNDLE_V4_1_PATH.open("wb") as f:
        pickle.dump(bundle, f)

    print(f"  saved model bundle: {MODEL_BUNDLE_V4_1_PATH}")
    print(f"  saved full person embeddings: {person_emb_output_path}")
    return bundle


def save_validation_outputs(validation: Dict[str, object]) -> None:
    validation["question_metrics"].to_csv(VALIDATION_BY_QUESTION_V4_1_PATH, index=False)
    validation["block_metrics"].to_csv(VALIDATION_BY_BLOCK_V4_1_PATH, index=False)
    validation["scored_df"].to_csv(HOLDOUT_PREDICTIONS_V4_1_PATH, index=False)
    validation["calibration_sweep"].to_csv(CALIBRATION_SWEEP_V4_1_PATH, index=False)
    CALIBRATION_PARAMS_V4_PATH.write_text(
        json.dumps(validation["calibration_params"], indent=2, sort_keys=True),
        encoding="utf-8",
    )
    VALIDATION_SUMMARY_V4_1_PATH.write_text(
        json.dumps(
            {
                "pre_calibration": validation["pre_summary"],
                "post_calibration": validation["summary"],
                "calibration_params": validation["calibration_params"],
            },
            indent=2,
            sort_keys=True,
        ),
...
def write_training_report(validation: Dict[str, object]) -> None:
    pre_summary = validation["pre_summary"]
    summary = validation["summary"]
    block_metrics = validation["block_metrics"]

    lines = [
        "TOLENDI V4.1 TRAINING REPORT",
        "=" * 40,
        "",
        f"Embedding K: {summary['person_embedding_k']}",
        f"Best stretch factor: {summary['best_stretch_factor']}",
        "",
        "Version comparison:",
        "  V1: accuracy=0.7244, mean_r=0.2587, variance_ratio=0.2883",
        "  V2: accuracy=0.7303, mean_r=0.3009, variance_ratio=0.3786",
        "  V3: accuracy=0.7300, mean_r=0.3057, variance_ratio=0.3834",
        (
            f"  V4.1 pre-cal: accuracy={pre_summary['accuracy']:.4f}, "
            f"mean_r={pre_summary['mean_pearson_r']:.4f}, "
            f"variance_ratio={pre_summary['variance_ratio']:.4f}"
        ),
        (
            f"  V4.1 calibrated: accuracy={summary['accuracy']:.4f}, "
            f"mean_r={summary['mean_pearson_r']:.4f}, "
    ...
    lines.extend(["", "Calibration sweep:"])
    for _, row in validation["calibration_sweep"].sort_values("stretch_factor").iterrows():
        lines.append(
            f"  stretch={row['stretch_factor']:.1f}: "
    ...
    lines.extend(
        [
            ...
            f"  Accuracy < 0.70: {summary['accuracy'] < 0.70}",
        ]
    )

    TRAINING_REPORT_V4_1_PATH.write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    try:
        ...
        print("\n[5/8] Writing report")
        write_training_report(validation)

        print("\n[6/8] Key outputs")
        print(f"  summary: {VALIDATION_SUMMARY_V4_1_PATH}")
        print(f"  by block: {VALIDATION_BY_BLOCK_V4_1_PATH}")
        print(f"  by question: {VALIDATION_BY_QUESTION_V4_1_PATH}")
        print(f"  holdout predictions: {HOLDOUT_PREDICTIONS_V4_1_PATH}")
        print(f"  bundle: {MODEL_BUNDLE_V4_1_PATH}")
        print(f"  calibration sweep: {CALIBRATION_SWEEP_V4_1_PATH}")
        print(f"  calibration params: {CALIBRATION_PARAMS_V4_PATH}")
        print(f"  report: {TRAINING_REPORT_V4_1_PATH}")

        print("\n[7/8] Validation snapshot")
        print(f"  pre-cal accuracy: {validation['pre_summary']['accuracy']:.4f}")
        print(f"  pre-cal mean Pearson r: {validation['pre_summary']['mean_pearson_r']:.4f}")
        print(f"  post-cal accuracy: {validation['summary']['accuracy']:.4f}")
        print(f"  post-cal mean Pearson r: {validation['summary']['mean_pearson_r']:.4f}")
        print(f"  post-cal variance ratio: {validation['summary']['variance_ratio']:.4f}")
